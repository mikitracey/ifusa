...
if (!class_exists('ThemeToolkit')) {
  class ThemeToolkit{
    ...
    function add_menu() {
      ...
      add_theme_page('Configure ' . $this->infos['theme_name'], /*$this->infos['theme_name']*/'Tigarator', 'edit_themes', basename($this->file), array(&$this,'admin_menu'));
    ...
    function read_options() {
      ...
      $options['_________junk-entry________'] = 'Hello world!';
    ...
    function admin_menu () {
      ...
      foreach ($this->infos['menu_options'] as $key=>$val) {
        ...
        if (@$items) {
          ...
          switch ($type) {
          case 'separator':
            print '<hr /><h3>'.$matches[1]."</h3><hr /></th>\n<td>&nbsp;</td>";
            break;
          case 'radio':
            print $matches[1]."</th>\n<td>";
            while ($items) {
